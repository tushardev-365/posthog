...
/// the cargo registry / git checkouts (any CARGO_HOME layout, not just
/// `~/.cargo`), or the standard library (`/rustc/<commit-hash>/...`, plus the
/// remapped/rustup `library/<crate>/src/` layouts). Every pattern is anchored
/// to a directory shape only build tooling produces; a directory the user
/// happened to name `vendor` or `rustc` must not match, since demoting real
/// app frames degrades fingerprinting for every customer. Paths outside these
/// locations stay whatever the client said — application source layouts vary
/// too much to classify positively.
...
mod test {
    ...
    fn resolved_library_frames_are_demoted_from_in_app() {
        ...
        // checkouts that merely contain a `vendor` or `rustc` path segment
        // must keep their client classification.
        let app_paths = [
